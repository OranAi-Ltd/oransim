...
class Node:
    ...
    cache_key_fn: Optional[Callable[..., str]] = None  # custom hashing
    description: str = ""
...
class CausalGraph:
    ...
    def node(self, name: str, fn: Callable, deps: List[str] = None,
             description: str = "", parallel_safe: bool = True) -> "CausalGraph":
        if name in self._nodes:
            raise ValueError(f"node already exists: {name}")
        self._nodes[name] = Node(
            name=name, fn=fn, deps=deps or [],
            description=description, parallel_safe=parallel_safe,
        )
        return self

    # ---- Topology ----

    def _topo_order(self, targets: List[str], known_inputs: Optional[Set[str]] = None) -> List[str]:
    ...
    def run(self, *, targets: Optional[List[str]] = None,
        ...
        order = self._topo_order(targets, known_inputs=set(inputs.keys()) | set(interventions.keys()))
        ...
        while True:
            ...
            def run_node(name: str) -> Tuple[str, Any, NodeTrace]:
                ...
                if name in interventions:
                    ...
                    tr = NodeTrace(
                        name=name, started_at=t0, duration_ms=0,
                        cache_hit=False, intervened=True, deps=node.deps,
                ...
                if name in inputs:
                    val = inputs[name]
                    return name, val, NodeTrace(name, t0, 0, False, False, node.deps,
                                                _safe_hash(val)[:80])
                # Build kwargs from deps
                kwargs = {d: results[d] for d in node.deps}
                # Cache key
                if use_cache and name not in dirty:
                    key = node.cache_key_fn(**kwargs) if node.cache_key_fn \
                          else _safe_hash(kwargs)
                    with self._cache_lock:
                        cached = self._cache.get((name, key))
                    if cached is not None:
                        self.cache_hits += 1
                        return name, cached, NodeTrace(
                            name, t0, (time.time()-t0)*1000, True, False,
                            node.deps, _safe_hash(cached)[:80])
                    self.cache_misses += 1
                else:
                    key = None
                # Compute
                ...
                tr = NodeTrace(
                    name=name, started_at=t0,
                    duration_ms=(time.time() - t0) * 1000,
                    cache_hit=False, intervened=False, deps=node.deps,
                    output_summary=_safe_hash(val)[:80],
                )
                return name, val, tr
            ...
            if parallel and len(ready) > 1:
                ...
                for n in seq:
                    ...
                    results[name] = val; traces.append(tr); executed.add(name)
            else:
                for n in ready:
                    name, val, tr = run_node(n)
                    results[name] = val; traces.append(tr); executed.add(name)
    ...
    def intervene(self, *, targets: List[str], do: Dict[str, Any],
                  inputs: Dict[str, Any] = None) -> GraphRun:
        """Pearl's do() at graph level: replaces node values, invalidates downstream cache."""
        return self.run(targets=targets, inputs=inputs or {},
                        interventions=do, use_cache=True, parallel=True)
    ...
    def trace_to_dict(self, run: GraphRun) -> Dict:
        return {
            ...
            "traces": [
                {"name": t.name, "duration_ms": round(t.duration_ms, 1),
                 "cache_hit": t.cache_hit, "intervened": t.intervened,
                 "deps": t.deps, "output": t.output_summary}
                for t in run.traces
            ],
        }
