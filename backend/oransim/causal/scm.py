...
NODES: List[SCMNode] = [
    ...
    SCMNode("public_sentiment", "L1", "exogenous", "全民舆情", time_varying=True,
             computed_by="world_events.py"),
    SCMNode("competitor_action", "L1", "exogenous", "竞品动作", time_varying=True),
    SCMNode("regulatory_state", "L1", "exogenous", "监管状态",
             description="广告法/平台审核紧/松"),
    SCMNode("supply_state", "L1", "exogenous", "供应链状态",
             description="缺货/正常/过剩"),

    # ---- L2 Brand (latent stocks) ----
    SCMNode("brand_awareness", "L2", "state", "品牌认知度", time_varying=True),
    SCMNode("brand_recall", "L2", "state", "品牌回忆", time_varying=True,
             computed_by="brand_memory.py"),
    SCMNode("brand_favor", "L2", "state", "品牌好感", time_varying=True,
             computed_by="brand_memory.py"),
    SCMNode("brand_aversion", "L2", "state", "品牌反感", time_varying=True,
             computed_by="brand_memory.py"),
    ...
    SCMNode("bid_strategy", "L3", "decision", "出价策略", intervenable=True,
             description="CPM/CPC/CPA/oCPX"),
    SCMNode("frequency_cap", "L3", "decision", "频次上限", intervenable=True),
    SCMNode("pacing", "L3", "decision", "投放节奏", intervenable=True,
             description="均匀/前置/后置"),
    ...
    SCMNode("impression_dist", "L4", "mediator", "曝光分发",
             computed_by="world_model.py"),
    SCMNode("recsys_amplification", "L4", "mediator", "算法放大",
             computed_by="recsys_rl.py", description="冷启破圈系数"),
    SCMNode("exposure_count", "L4", "mediator", "曝光次数", time_varying=True),
    SCMNode("unique_reach", "L4", "mediator", "唯一覆盖",
             computed_by="cross_platform.py"),
    SCMNode("frequency", "L4", "mediator", "曝光频次",
             computed_by="cross_platform.py"),
    SCMNode("audience_match", "L4", "mediator", "人群匹配度",
             computed_by="world_model.py"),
    SCMNode("organic_amplification", "L4", "mediator", "自然扩散",
             computed_by="hawkes.py", description="Hawkes 二次曝光"),

    # ---- L5 User State ----
    SCMNode("attention", "L5", "state", "注意力"),
    SCMNode("user_fatigue", "L5", "state", "用户疲劳", time_varying=True,
             computed_by="cross_platform.py"),
    SCMNode("user_mood", "L5", "state", "用户情绪", time_varying=True),
    SCMNode("comprehension", "L5", "state", "信息理解度"),
    SCMNode("prior_belief", "L5", "state", "先验态度",
             description="刷之前对品牌的认知"),

    # ---- L6 Discourse / Social ----
    SCMNode("comment_sentiment", "L6", "mediator", "评论情绪",
             computed_by="discourse.py"),
    SCMNode("group_consensus", "L6", "mediator", "群聊共识",
             computed_by="group_chat.py"),
    SCMNode("group_polarization", "L6", "mediator", "群体极化",
             computed_by="group_chat.py"),
    SCMNode("peer_influence", "L6", "mediator", "同伴影响"),
    SCMNode("viral_score", "L6", "mediator", "病毒系数",
             computed_by="hawkes.py"),

    # ---- L7 Funnel ----
    SCMNode("ad_recall", "L7", "mediator", "广告回忆"),
    SCMNode("click", "L7", "mediator", "点击", computed_by="statistical.py"),
    SCMNode("engagement", "L7", "mediator", "互动",
    ...
    SCMNode("conversion", "L7", "mediator", "下单转化",
             computed_by="statistical.py"),
...
EDGES: List[Tuple[str, str]] = [
    # ---- L1 → others ----
    ("macro_env", "user_mood"), ("macro_env", "brand_equity"),
    ("weather", "user_mood"), ("weather", "site_visit"),
    ("season", "ecpm_bid"), ("season", "click"),
    ("holiday_calendar", "ecpm_bid"), ("holiday_calendar", "click"),
    ("holiday_calendar", "conversion"),
    ("dow", "user_mood"), ("dow", "frequency"),
    ...
    # ---- L2 brand → user ----
    ("brand_awareness", "ad_recall"), ("brand_awareness", "click"),
    ("brand_recall", "click"), ("brand_recall", "ad_recall"),
    ("brand_favor", "click"), ("brand_favor", "conversion"),
    ("brand_aversion", "click"),
    ("prior_purchase", "repurchase"), ("prior_purchase", "click"),
    ...
    ("attention", "ad_recall"), ("attention", "click"),
    ("user_fatigue", "click"), ("user_fatigue", "engagement"),
    ("user_mood", "click"), ("user_mood", "conversion"),
    ("comprehension", "click"), ("comprehension", "conversion"),
    ("prior_belief", "click"), ("prior_belief", "conversion"),

    # ---- L6 discourse → second-wave funnel ----
    ("comment_sentiment", "click"), ("comment_sentiment", "conversion"),
    ...
    ("peer_influence", "click"), ("peer_influence", "engagement"),
    ...
    ("brand_lift_intent", "purchase_intent_short_term"
        if False else "conversion"),    # placeholder
    ("ltv_increment", "roi"),
...
LAYER_LABELS = {
    "L1": "宏观/外生", "L2": "品牌存量", "L3": "投放决策",
    "L4": "算法分发", "L5": "用户状态", "L6": "群体话语",
    "L7": "漏斗", "L8": "产出",
}
LAYER_COLOR = {
    "L1": "#8b93a7", "L2": "#ffc857", "L3": "#6ea8fe",
    "L4": "#bd93f9", "L5": "#8be9fd", "L6": "#ff79c6",
    "L7": "#5ed39b", "L8": "#ff7a85",
}
...
def dag_dict() -> Dict:
    ...
    return {
        ...
        "nodes": [
            {
                "name": n.name, "label_zh": n.label_zh,
                "layer": n.layer, "layer_label": LAYER_LABELS[n.layer],
                "category": n.category,
        ...
        "layers": [
            {"id": L, "label": LAYER_LABELS[L], "color": LAYER_COLOR[L],
