...
def compute_cate(
    ...
    delta = np.array([cf_click_prob.get(i, 0.0) - baseline_click_prob.get(i, 0.0)
                      for i in union], dtype=np.float32)
    ...
    if float(np.abs(delta).max()) < 1e-9:
        return [{
            ...
            "note": (f"base 触达 {len(base_keys)} / cf 触达 {len(cf_keys)} / 交集 "
