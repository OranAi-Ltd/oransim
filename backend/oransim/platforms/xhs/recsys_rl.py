...
from .world_model_legacy import PlatformWorldModel, AudienceFilter, ImpressionResult
...
class RecSysRLSimulator:
    ...
    def _score(self, creative: Creative, platform: str, weights: np.ndarray,
               aud_filter=None, kol: Optional[KOL] = None, rng_seed: int = 0) -> np.ndarray:
    ...
    def simulate(
        ...
        for r in range(n_rounds):
            ...
            # score everyone
            score = self._score(creative, platform, w, audience_filter, kol,
                                rng_seed=seed + r * 17)
            ...
            rounds.append(RLRoundResult(
                round_id=r, weights=w.copy(), impression_idx=idx.astype(np.int64),
                click_rate=click_rate, engage_rate=engage_rate,
            ...
            feats_idx = np.stack([
                ...
                np.ones(len(idx), dtype=np.float32) if kol is None
        ...
        if all_idx:
            ...
            cum_imp = ImpressionResult(
                agent_idx=idx_arr, weight=w_arr,
                total_impressions=float(total_imps), platform=platform,
                score_breakdown={
                    "content": (self.pop.interest[idx_arr] @ creative.content_emb + 1) / 2,
                    "platform_activity": self.pop.platform_activity[idx_arr,
                        self.wm.platform_idx.get(platform, 0)],
                    "audience_filter": np.ones(len(idx_arr), dtype=np.float32),
                    "kol_boost": np.ones(len(idx_arr), dtype=np.float32) if kol is None
            ...
            cum_imp = ImpressionResult(
                ...
                total_impressions=0.0, platform=platform,
                score_breakdown={"content": np.array([]), "platform_activity": np.array([]),
                                 "audience_filter": np.array([]), "kol_boost": np.array([])},
            )

        return RecSysRLReport(
            rounds=rounds, cumulative_impression=cum_imp,
            peak_round=peak_round, break_out=broke,
            final_weights=w,
        )
...
def rl_report_to_dict(rep: RecSysRLReport) -> Dict:
    return {
        ...
        "final_weights": {
            k: round(float(v), 3) for k, v in zip(
